...
if (galleryEl) {
  const gallerySwiper = new Swiper(galleryEl, {
    ...
    slidesPerView: "auto",
    spaceBetween: 0,      // overlap го правим с CSS “стълбички”
    speed: 500,
    grabCursor: true,
  ...
  // wheel само върху галерията (без да убива page scroll-а)
...
// --- mark 2nd neighbors (far stairs) so we can style them ---
function markFarSlides(swiper) {
  swiper.slides.forEach(s => s.classList.remove("g-far-prev", "g-far-next"));

  const active = swiper.slides[swiper.activeIndex];
  if (!active) return;

  // helper to find the next/prev slide element in DOM order
  const next1 = active.nextElementSibling;
  const prev1 = active.previousElementSibling;
...
markFarSlides(gallerySwiper);
gallerySwiper.on("slideChangeTransitionStart", () => markFarSlides(gallerySwiper));
gallerySwiper.on("loopFix", () => markFarSlides(gallerySwiper));
